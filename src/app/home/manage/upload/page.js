      ...
      setIconMetadata((prev) =>
        prev.map((icon) => {
          if (selectedIcons.includes(icon.id)) {
            return { ...icon, categories: bulkCategory };
          }
          return icon;
        })
      );
    }
  }, [bulkCategory, selectedIcons]);

  useEffect(() => {
    // if (typeof bulkIconType == "string") {
      setIconMetadata((prev) =>
        prev.map((icon) => {
          if (selectedIcons.includes(icon.id)) {
            return { ...icon, iconType: bulkIconType };
          }
          return icon;
        })
      );
    // }
  }, [bulkIconType, selectedIcons]);
  ...
  const handleUpload = async () => {
    ...
    uploadIcons(
      ...
      if (!response.error) {
        ...
        const tempMetadata = [...iconMetadata];
        response.data.id.map((id, index) => {
          tempIcons[index].status = "completed";
          tempIcons[index].id = id;
          tempMetadata[index].id = id;
        });

        setIcons(tempIcons);

        setIconMetadata(tempMetadata);
